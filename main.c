...
//используется для преобразования входной последовательности в ОПЗ
//и расчета по ней
//не следит за удалением множеств входящих в состав лексем
...
//считывает входную строку до символа end
//выделяет динамическую память под строку
...
//инициализирует динамический массив, и заносит в него элементы входной строки в виде ОПЗ
//проверяет корректность входных данных
int input(struct DynamicArray* dest);

//инициализирует динамический массив, и заносит в него элементы входной строки,
//проверяет корректность входных данных
int inputSet(struct Set* dest);
...
int performOperation(int(*operation)(struct Set, struct Set, struct Set*),
                     struct Stack* stack, struct DynamicArray* dest);

//принимает массив с ОПЗ
...
int inputSet(struct Set* dest) {
    ...
    while (cur_ch != EOF && cur_ch != '\n' && cur_ch != END_SET_CH) {
        if (cur_ch == BORDER_ELEM_CH) {
            char* elem = readString(BORDER_ELEM_CH);
            if (elem == NULL) {
                deleteSet(&res);
                return 0;
            }
...
int count(const struct DynamicArray exp, struct Set *dest) {
    ...
    struct Stack stack;
    initStack(&stack);
    // так как за удаление лексем-множеств отвечает только DynamicArray, все промежуточные множества,
    // получаемые при расчетах будут заноситься в tmp_sets, чтобы быть удаленными в конце расчетов
    ...
    for (i = 0; i < exp.size; ++i) {
        ...
        case UNION: {
            int res = performOperation(unionSet, &stack, &tmp_sets);
            if (!res) {
                deleteStack(&stack);
                deleteSet(dest);
                deleteDArray(&tmp_sets);
        ...
        case INTERSECTION: {
            int res = performOperation(intersectionSet, &stack, &tmp_sets);
            if (!res) {
                deleteStack(&stack);
                deleteSet(dest);
                deleteDArray(&tmp_sets);
        ...
        case COMPLEMENT: {
            int res = performOperation(complementSet, &stack, &tmp_sets);
            if (!res) {
                deleteStack(&stack);
                deleteSet(dest);
                deleteDArray(&tmp_sets);
...
int removeSet(struct Set* set,char* data) {
    struct SetNode* cur_node = set->head;
    while (cur_node != NULL) {
        int res = strcmp(data, cur_node->str);
        if (res == 0) {
